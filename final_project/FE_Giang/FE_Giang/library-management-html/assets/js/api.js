...
class HttpClient {
  ...
  async handleResponse(response) {
    ...
    if (!response.ok) {
      ...
      // Handle other errors
      let errorMessage = data.message || data.error || data || 'Request failed';
      
      // Convert "Bad credentials" to user-friendly Vietnamese message
    ...
    try {
      ...
      if (typeof response === 'string' && response.startsWith('eyJ')) {
        ...
        try {
          ...
          const fullName = payload.fullName || payload.sub || payload.email;
          user = {
...
const BooksAPI = {
   ...
   * @returns {Promise<Object>} PageResponse with { content, pageNumber, pageSize, totalElements, totalPages, last, first }
   */
  async getAll(page = 0, size = 10) {
    try {
      const response = await http.get(`/api/books?page=${page}&size=${size}`);
      // Return PageResponse structure
   ...
   * USE THIS instead of getAll() when you need search/sort functionality
   * GET /api/books?page=0&size=10&sortBy=title&sortDirection=ASC&search=query
  ...
  async getAllWithParams(params = {}) {
    try {
      ...
      const response = await http.get(`/api/books?${queryParams.toString()}`);
...
const AdminAPI = {
  ...
  async getAllUsers(page = 0, size = 10) {
    try {
      const response = await http.get(`/admin/users?page=${page}&size=${size}`);
      // Return PageResponse structure
      if (response && response.content) {
        return response;
      }
